...
# and launches the 90-model ASRNN verification ensemble for Morse.
#
# Usage (from within this folder):
#   ./run_cluster_job.sh                      # defaults: workers=1, ensemble=10
#   WORKERS=4 ENSEMBLE=10 ./run_cluster_job.sh
...
OUT_DIR="${OUT_DIR:-./Baseline_ASRNN_Verified}"
...
if torch.cuda.is_available():
    ...
    print("See RUN_INSTRUCTIONS.md's troubleshooting section for a CUDA-matched torch install.")
PY
echo "------------------"

echo "Launching training: workers=$WORKERS ensemble=$ENSEMBLE out_dir=$OUT_DIR"
python train_asrnn_ensemble.py --workers "$WORKERS" --ensemble "$ENSEMBLE" --out-dir "$OUT_DIR"
