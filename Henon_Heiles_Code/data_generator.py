...
def main():
    ...
    dtype        = torch.float32
    alpha1s      = torch.tensor([0.2, 0.4, 0.6, 0.8], dtype=dtype, device=device)
    alpha2s      = torch.tensor([0.2, 0.4, 0.6, 0.8], dtype=dtype, device=device)
    a1g, a2g     = torch.meshgrid(alpha1s, alpha2s, indexing='xy')
    alphas_train = torch.stack((a1g.reshape(-1), a2g.reshape(-1)), dim=1) 
    splits       = 100
    T_window     = 15                 
